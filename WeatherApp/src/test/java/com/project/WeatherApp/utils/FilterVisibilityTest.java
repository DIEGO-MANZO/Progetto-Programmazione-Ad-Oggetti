...
class FilterVisibilityTest {
	...
	void oneDay() {
		...
		WrongValueException e = assertThrows(WrongValueException.class, () -> {
			filter.oneDay(cities, "m");
		});

		assertEquals("m è una stringa errata! Devi inserire una stringa tra max/MAX/Max oppure min/MIN/Min",
				e.getMex());
	...
	void fiveDay() {
		...
		WrongValueException e = assertThrows(WrongValueException.class, () -> {
			filter.fiveDay(cities, "k");
		});

		assertEquals("k è una stringa errata! Devi inserire una stringa tra max/MAX/Max oppure min/MIN/Min",
				e.getMex());
